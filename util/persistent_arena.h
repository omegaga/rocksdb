...
class PersistentArena : public Allocator {
 public:
  // No copying allowed
  PersistentArena(const PersistentArena&) = delete;
  void operator=(const PersistentArena&) = delete;
  ...
  explicit PersistentArena(size_t block_size = kMinBlockSize,
...
inline char* PersistentArena::Allocate(size_t bytes) {
