...
// PersistentConcurrentArena wraps an Arena.  It makes it thread safe using a fast
...
// only if PersistentConcurrentArena actually notices concurrent use, and they
...
class PersistentConcurrentArena : public Allocator {
 public:
  ...
  explicit PersistentConcurrentArena(size_t block_size = Arena::kMinBlockSize,
  ...
  inline uintptr_t AbsToRel(const void* abs) {
    return arena_.AbsToRel(abs);
  }

  inline void* RelToAbs(uintptr_t rel) {
    return arena_.RelToAbs(rel);
  }
 ...
 private:
  ...
  PersistentConcurrentArena(const PersistentConcurrentArena&) = delete;
  PersistentConcurrentArena& operator=(const PersistentConcurrentArena&) = delete;
};
