...
#include "util/persistent_arena.h"
...
#ifndef OS_WIN
const size_t PersistentArena::kInlineSize;
#endif

const size_t PersistentArena::kMinBlockSize = 4096;
const size_t PersistentArena::kMaxBlockSize = 2u << 30;
static const int kAlignUnit = sizeof(void*);

PersistentArena::PersistentArena(size_t block_size, size_t huge_page_size)
...
char* PersistentArena::AllocateFallback(size_t bytes, bool aligned) {
...
char* PersistentArena::AllocateFromHugePage(size_t bytes) {
#ifdef MAP_HUGETLB
  if (hugetlb_size_ == 0) {
    return nullptr;
  }
...
char* PersistentArena::AllocateAligned(size_t bytes, size_t huge_page_size,
                                       Logger* logger) {
...
char* PersistentArena::AllocateNewBlock(size_t block_bytes) {
  ...
  // FIXME(jhli): Use persistent malloc
