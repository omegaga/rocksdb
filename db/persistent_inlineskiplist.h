...
#include "util/persistent_concurrent_arena.h"

namespace rocksdb {

template <class Comparator>
class PersistentInlineSkipList {
 private:
  struct Node;

 public:
  // Create a new PersistentInlineSkipList object that will use "cmp" for comparing
  ...
  // skiplist object.
  explicit PersistentInlineSkipList(Comparator cmp, PersistentConcurrentArena* allocator,
  ...
  class Iterator {
   public:
    ...
    explicit Iterator(const PersistentInlineSkipList* list);

    // Change the underlying skiplist used for this iterator
    // This enables us not changing the iterator without deallocating
    // an old one and then allocating a new one
    void SetList(const PersistentInlineSkipList* list);
   ...
   private:
    const PersistentInlineSkipList* list_;
    Node* node_;
 ...
 private:
  ...
  PersistentConcurrentArena* const allocator_;  // Allocator used for allocations of nodes
  ...
  // No copying allowed
  PersistentInlineSkipList(const PersistentInlineSkipList&);
  PersistentInlineSkipList& operator=(const PersistentInlineSkipList&);
};
...
template <class Comparator>
struct PersistentInlineSkipList<Comparator>::Node {
  ...
  Node* Next(int n, PersistentConcurrentArena* allocator) {
    assert(n >= 0);
    // Use an 'acquire load' so that we observe a fully initialized
    // version of the returned Node.
    return reinterpret_cast<Node*>(allocator->RelToAbs(next_[-n].load(std::memory_order_acquire)));
  }

  void SetNext(int n, Node* x, PersistentConcurrentArena* allocator) {
    assert(n >= 0);
    // Use a 'release store' so that anybody who reads through this
    // pointer observes a fully initialized version of the inserted node.
    next_[-n].store(allocator->AbsToRel(x), std::memory_order_release);
  }

  bool CASNext(int n, Node* expected, Node* x, PersistentConcurrentArena* allocator) {
    assert(n >= 0);
    uintptr_t expected_rel = allocator->AbsToRel(expected);
    return next_[-n].compare_exchange_strong(expected_rel, allocator->AbsToRel(x));
  }

  // No-barrier variants that can be safely used in a few locations.
  Node* NoBarrier_Next(int n, PersistentConcurrentArena* allocator) {
    assert(n >= 0);
    return reinterpret_cast<Node*>(allocator->RelToAbs(next_[-n].load(std::memory_order_relaxed)));
  }

  void NoBarrier_SetNext(int n, Node* x, PersistentConcurrentArena* allocator) {
    assert(n >= 0);
    next_[-n].store(allocator->AbsToRel(x), std::memory_order_relaxed);
  }
...
template <class Comparator>
inline PersistentInlineSkipList<Comparator>::Iterator::Iterator(
    const PersistentInlineSkipList* list) {
  SetList(list);
}

template <class Comparator>
inline void PersistentInlineSkipList<Comparator>::Iterator::SetList(
    const PersistentInlineSkipList* list) {
  list_ = list;
  node_ = nullptr;
}
...
template <class Comparator>
inline bool PersistentInlineSkipList<Comparator>::Iterator::Valid() const {
  return node_ != nullptr;
}

template <class Comparator>
inline const char* PersistentInlineSkipList<Comparator>::Iterator::key() const {
  assert(Valid());
  return node_->Key();
}

template <class Comparator>
inline void PersistentInlineSkipList<Comparator>::Iterator::Next() {
  assert(Valid());
  node_ = node_->Next(0, list_->allocator_);
}

template <class Comparator>
inline void PersistentInlineSkipList<Comparator>::Iterator::Prev() {
...
template <class Comparator>
inline void PersistentInlineSkipList<Comparator>::Iterator::Seek(const char* target) {
  node_ = list_->FindGreaterOrEqual(target);
}

template <class Comparator>
inline void PersistentInlineSkipList<Comparator>::Iterator::SeekToFirst() {
  node_ = list_->head_->Next(0, list_->allocator_);
}

template <class Comparator>
inline void PersistentInlineSkipList<Comparator>::Iterator::SeekToLast() {
...
template <class Comparator>
int PersistentInlineSkipList<Comparator>::RandomHeight() {
...
template <class Comparator>
bool PersistentInlineSkipList<Comparator>::KeyIsAfterNode(const char* key,
                                                Node* n) const {
...
template <class Comparator>
typename PersistentInlineSkipList<Comparator>::Node*
PersistentInlineSkipList<Comparator>::FindGreaterOrEqual(const char* key) const {
  ...
  while (true) {
    Node* next = x->Next(level, allocator_);
...
template <class Comparator>
typename PersistentInlineSkipList<Comparator>::Node*
PersistentInlineSkipList<Comparator>::FindLessThan(const char* key, Node** prev) const {
  Node* x = head_;
  int level = GetMaxHeight() - 1;
...
template <class Comparator>
typename PersistentInlineSkipList<Comparator>::Node*
PersistentInlineSkipList<Comparator>::FindLast() const {
  Node* x = head_;
  int level = GetMaxHeight() - 1;
  while (true) {
    Node* next = x->Next(level, allocator_);
...
template <class Comparator>
uint64_t PersistentInlineSkipList<Comparator>::EstimateCount(const char* key) const {
  uint64_t count = 0;
...
template <class Comparator>
PersistentInlineSkipList<Comparator>::PersistentInlineSkipList(const Comparator cmp,
                                           PersistentConcurrentArena* allocator,
                                           int32_t max_height,
  ...
  if (!restore) {
    ...
    for (int i = 0; i < kMaxHeight_; i++) {
      head_->SetNext(i, nullptr, allocator_);
      prev_[i] = head_;
    }
    allocator->Init([this](uintptr_t* nowPtr) {
    });
  } else {
    allocator->Recover([this, max_height](uintptr_t* recoverPtr) {
      // Set head_
      Node* ptr = reinterpret_cast<Node*>(*recoverPtr);
      auto prefix = sizeof(std::atomic<Node*>) * (max_height - 1);
      head_ = reinterpret_cast<Node*>(ptr + prefix);
      ptr += prefix + sizeof(Node);

      // Set prev_
      prev_ = reinterpret_cast<Node**>(ptr);
      ptr += sizeof(Node*) * kMaxHeight_;

      // Set recoverPtr
      *recoverPtr = reinterpret_cast<uintptr_t>(ptr);
    });
  }
}

template <class Comparator>
char* PersistentInlineSkipList<Comparator>::AllocateKey(size_t key_size) {
  return const_cast<char*>(AllocateNode(key_size, RandomHeight())->Key());
}

template <class Comparator>
typename PersistentInlineSkipList<Comparator>::Node*
PersistentInlineSkipList<Comparator>::AllocateNode(size_t key_size, int height) {
...
template <class Comparator>
void PersistentInlineSkipList<Comparator>::Insert(const char* key) {
  ...
  if (prev_height > 0 && !KeyIsAfterNode(key, prev_[0]->NoBarrier_Next(0, allocator_)) &&
  ...
  assert(prev_[0]->Next(0, allocator_) == nullptr || !Equal(key, prev_[0]->Next(0, allocator_)->Key()));
  ...
  for (int i = 0; i < height; i++) {
    ...
    x->NoBarrier_SetNext(i, prev_[i]->NoBarrier_Next(i, allocator_), allocator_);
    prev_[i]->SetNext(i, x, allocator_);
  }
  prev_[0] = x;
...
template <class Comparator>
void PersistentInlineSkipList<Comparator>::FindLevelSplice(const char* key, Node* before,
                                                 Node* after, int level,
                                                 Node** out_prev,
                                                 Node** out_next) {
  while (true) {
    Node* next = before->Next(level, allocator_);
...
template <class Comparator>
void PersistentInlineSkipList<Comparator>::InsertConcurrently(const char* key) {
  Node* x = reinterpret_cast<Node*>(const_cast<char*>(key)) - 1;
  int height = x->UnstashHeight();
  assert(height >= 1 && height <= kMaxHeight_);
  ...
  for (int i = 0; i < height; ++i) {
    while (true) {
      x->NoBarrier_SetNext(i, next[i], allocator_);
      if (prev[i]->CASNext(i, next[i], x, allocator_)) {
        // success
        break;
      }
...
template <class Comparator>
bool PersistentInlineSkipList<Comparator>::Contains(const char* key) const {
