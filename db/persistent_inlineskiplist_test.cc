...
typedef PersistentInlineSkipList<TestComparator> TestInlineSkipList;
...
TEST_F(InlineSkipTest, Empty) {
  PersistentConcurrentArena arena;
  TestComparator cmp;
  PersistentInlineSkipList<TestComparator> list(cmp, &arena);
  Key key = 10;
  ASSERT_TRUE(!list.Contains(Encode(&key)));

  PersistentInlineSkipList<TestComparator>::Iterator iter(&list);
  ASSERT_TRUE(!iter.Valid());
  iter.SeekToFirst();
  ASSERT_TRUE(!iter.Valid());
  key = 100;
...
class ConcurrentTest {
 ...
 private:
  ...
  PersistentConcurrentArena arena_;

  // PersistentInlineSkipList is not protected by mu_.  We just use a single writer
  // thread to modify it.
  PersistentInlineSkipList<TestComparator> list_;
 ...
 public:
  ...
  void ReadStep(Random* rnd) {
    ...
    PersistentInlineSkipList<TestComparator>::Iterator iter(&list_);
