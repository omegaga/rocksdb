...
public:
  explicit PersistentSkipListRep(const MemTableRep::KeyComparator& compare,
  ...
  virtual void Get(const LookupKey& k, void* callback_args,
    ...
    PersistentSkipListRep::Iterator iter(&skip_list_);
    Slice dummy_slice;
  ...
  virtual ~PersistentSkipListRep() override { }
  ...
  class LookaheadIterator : public MemTableRep::Iterator {
   public:
    explicit LookaheadIterator(const PersistentSkipListRep& rep) :
   ...
   private:
    const PersistentSkipListRep& rep_;
    InlineSkipList<const MemTableRep::KeyComparator&>::Iterator iter_;
  ...
  virtual MemTableRep::Iterator* GetIterator(Arena* arena = nullptr) override {
    if (lookahead_ > 0) {
      void *mem =
        arena ? arena->AllocateAligned(sizeof(PersistentSkipListRep::LookaheadIterator))
              : operator new(sizeof(PersistentSkipListRep::LookaheadIterator));
      return new (mem) PersistentSkipListRep::LookaheadIterator(*this);
    } else {
      void *mem =
        arena ? arena->AllocateAligned(sizeof(PersistentSkipListRep::Iterator))
              : operator new(sizeof(PersistentSkipListRep::Iterator));
      return new (mem) PersistentSkipListRep::Iterator(&skip_list_);
    }
  }
};
}
